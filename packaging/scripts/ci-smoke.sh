...
# STEP 9 / H2 (pip-audit, CycloneDX SBOM): runs in .github/workflows/ci.yml before dpkg-buildpackage — docs/sbom-signing-audit.md. This script is STEP 10 (H3): pytest includes SBOM/H2 doc contracts; it does not invoke pip-audit itself.
...
# Canonical local full-product installer: bash -n packaging/scripts/install-le-vibe-local.sh (STEP 14 / §7.3 — docs/LOCAL_INSTALL_ONE_SHOT.md).
...
echo "ci-smoke: canonical local installer (bash -n)"
