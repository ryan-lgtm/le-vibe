...
if [[ ! -e "$BIN" ]]; then
  ...
  echo "stage-le-vibe-ide-deb: or linux_compile tarball: packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh /path/to/vscodium-linux-build.tar.gz --yes (editor/BUILD.md 14.f)" >&2
  exit 1
fi
if [[ ! -x "$BIN" ]]; then
