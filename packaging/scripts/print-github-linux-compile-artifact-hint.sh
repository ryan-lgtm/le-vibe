...
echo "#"
echo "# --- C) curl API (no gh) — list runs/artifacts without token on public repos; download needs token ---"
echo "#    packaging/scripts/download-vscodium-linux-compile-artifact.sh [--install]"
echo "#    (writes vscodium-linux-build.tar.gz when GITHUB_TOKEN/GH_TOKEN set; discovery-only exits 2 with artifact id if no token)"
echo "#"
echo "# --- Confirm ---"
