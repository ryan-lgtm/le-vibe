...
echo "#"
echo "# --- C) curl API (no gh) — token with actions:read ---"
echo "#    GITHUB_TOKEN=... packaging/scripts/download-vscodium-linux-compile-artifact.sh [--install]"
echo "#    (writes vscodium-linux-build.tar.gz; --install → install-vscodium-linux-tarball-to-editor-vendor.sh)"
echo "#"
echo "# --- Confirm ---"
