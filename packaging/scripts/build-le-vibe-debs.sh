...
Artifacts:
  Stack:  typically ../le-vibe_*.deb (parent of repo root — standard dpkg-buildpackage); discovery helper: packaging/scripts/resolve-latest-le-vibe-stack-deb.sh.
...
Publishing / releases (H1):
  ...
  when tagging; Checklist shorthand (stack-only vs --with-ide vs Combined drop) — same doc (Pre-publish — Checklist shorthand).
  Stack vs IDE: debian/changelog (le-vibe stack) vs packaging/debian-le-vibe-ide/debian/changelog (le-vibe-ide) — same doc (IDE le-vibe-ide changelog).
...
Full-product (--with-ide): When both .deb files are produced, the script prints a Full-product install
  ...
  Preflight (all gaps at once): packaging/scripts/preflight-step14-closeout.sh [--require-stack-deb] [--skip-gate] — docs/PM_DEB_BUILD_ITERATION.md (*Preflight (all gaps)*).
...
find_stack_deb() {
  # Same discovery as verify-step14-closeout.sh --require-stack-deb (resolve-latest-le-vibe-stack-deb.sh).
  local p
  p="$("$ROOT/packaging/scripts/resolve-latest-le-vibe-stack-deb.sh" "$ROOT")"
  if [[ -n "$p" ]]; then
    echo "$p"
    return 0
  fi
...
if [[ -z "$STACK_DEB" || ! -f "$STACK_DEB" ]]; then
  echo "build-le-vibe-debs: could not locate le-vibe_*.deb (searched beside clone and repo root) — if dpkg-buildpackage failed, fix errors above; on success the stack .deb is usually ../le-vibe_*.deb from repo root (or copy into repo root)." >&2
else
...
if [[ "$WITH_IDE" -eq 1 && -n "$STACK_DEB" && -f "$STACK_DEB" && -n "$IDE_DEB" && -f "$IDE_DEB" ]]; then
  ...
  echo "    §7.3 preflight (all gaps before strict verifier): ./packaging/scripts/preflight-step14-closeout.sh --require-stack-deb"
