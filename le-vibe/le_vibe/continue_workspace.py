...
def _lvibe_continue_rule_body() -> str:
    return (
        ...
        "- **`session-manifest.json`** — `session_steps`, **`product.epics`** / tasks; iterate backlog per "
        "`docs/SESSION_ORCHESTRATION_SPEC.md` (in the Lé Vibe product repo). Canonical bundled shape: "
        "**`schemas/session-manifest.v1.example.json`**.\n"
        ...
        "- **`agents/<agent_id>/skill.md`** — per-role skill markdown (PM, DevOps, engineering, etc.).\n"
...
def _product_welcome_rule_body() -> str:
    ...
    return (
        ...
        "Open **`.lvibe/WELCOME.md`** in this workspace for the full welcome paragraph.\n"
    )
