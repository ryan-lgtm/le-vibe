...
**Paste-ready prompt (repo root):** `python3 packaging/scripts/print-master-iteration-loop-prompt.py` — copy **stdout** into Cursor. Same prompt for every hat; set `MODE:`, optional `OWNER_DIRECTIVES:`, and optional `CONTINUATION:` per [`docs/MASTER_ITERATION_LOOP.md`](../../docs/MASTER_ITERATION_LOOP.md) *Human workflow*. **Marching STEPs 0–17**, **anti-drift**, and **when to rerun** the print script: same doc (*Marching the full queue*, *When to rerun*).
