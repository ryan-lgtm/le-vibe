...
**CI:** [`.github/workflows/build-le-vibe-ide.yml`](../.github/workflows/build-le-vibe-ide.yml) runs on `workflow_dispatch`, **`workflow_call`**, **`pull_request`** (paths under **`editor/`**, IDE workflows, **`ci-editor-gate`**, **`ci-vscodium-bash-syntax`**), and tags `ide-v*`. [`.github/workflows/build-linux.yml`](../.github/workflows/build-linux.yml) is a **`build-linux`** label that dispatches the same job (manual only). When upstream is missing, the job **passes** with a skip notice; with the VSCodium submodule, smoke includes **`bash -n`** on core scripts via [`ci-vscodium-bash-syntax.sh`](../packaging/scripts/ci-vscodium-bash-syntax.sh). Extend the workflow with the real Linux build and **`.deb`/bundle** upload per **`docs/vscodium-fork-le-vibe.md`**; today the job uploads a small **metadata** artifact (`ide-ci-metadata.txt`, pinned **`vscode-upstream-stable.json`** when the submodule is present) so the artifact path is wired before binaries exist. **Local check:** [`packaging/scripts/ci-editor-gate.sh`](../packaging/scripts/ci-editor-gate.sh) (also run via **[`packaging/scripts/ci-smoke.sh`](../packaging/scripts/ci-smoke.sh)**).
