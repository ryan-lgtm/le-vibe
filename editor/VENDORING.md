...
Then follow upstream’s **prepare / build** docs from **`editor/vscodium/`**. CI [`.github/workflows/build-le-vibe-ide.yml`](../.github/workflows/build-le-vibe-ide.yml) treats either **`editor/package.json`** (flat checkout) or **`editor/vscodium/package.json`** (submodule layout) as “sources present.”
