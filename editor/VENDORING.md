...
**Master orchestrator order:** **0 → 1 → 14 → 2–13 → 15–17** — checking out **`editor/vscodium/`** is prerequisite for **STEP 14** / **§7.3** immediately after baseline **STEP 0–1**. See **[`docs/PROMPT_BUILD_LE_VIBE.md`](../docs/PROMPT_BUILD_LE_VIBE.md)** (*ORDERED WORK QUEUE*, *Rolling iteration — prefer continuation*) and **[`docs/PM_STAGE_MAP.md`](../docs/PM_STAGE_MAP.md)** *Execution order* / **STEP 16** (same pointer as **[`README.md`](README.md)** *Master orchestrator order*).
...
**Partial tree / CI tarball without a full local `dev/build.sh`:** **`../packaging/scripts/print-github-linux-compile-artifact-hint.sh`**, **`../packaging/scripts/trigger-le-vibe-ide-linux-compile.sh`**, **`../packaging/scripts/download-vscodium-linux-compile-artifact.sh`** **`--install`**, **`../packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh`** — **[`BUILD.md`](BUILD.md)** (*Partial tree*, **14.f**); same maintainer family as **`preflight-step14-closeout.sh`** / **`lvibe ide-prereqs --print-closeout-commands`**.
