...
| **Icons (stack vs IDE)** | [`packaging/icons/`](../../packaging/icons/) → **[`sync-linux-icon-assets.sh`](sync-linux-icon-assets.sh)** → **`editor/vscodium/src/stable/resources/linux/le-vibe.{svg,png}`** (untracked; superproject **`.gitmodules`** **`ignore = untracked`**) **and** **`src/{stable,insider}/src/vs/workbench/browser/media/code-icon.svg`** (packaging mark, 1024×1024; overwrites upstream **tracked** files in the submodule) | [`docs/brand-assets.md`](../../docs/brand-assets.md); **`product-branding-merge.json`** **`linuxIconName`**. |
