...
- **Preflight (local):** canonical package list — **`packaging/linux-vscodium-ci-apt.pkgs`** (same as **`build-le-vibe-ide.yml`** *Install Linux build dependencies*). Verify with **`./packaging/scripts/check-linux-vscodium-build-deps.sh`** (**`pkg-config xkbfile`** included); install with **`./packaging/scripts/install-linux-vscodium-build-deps.sh`** (requires **`sudo`**). **`packaging/scripts/ci-vscodium-linux-dev-build.sh`** runs the check automatically before §7.3 merge/sync (full compile only; skipped when **`LEVIBE_VSCODIUM_PREPARE_ONLY=1`** or **`LEVIBE_SKIP_HOST_DEPS_CHECK=1`**).
