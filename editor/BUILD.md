...
1. **Toolchain (14.a):** **[`.nvmrc`](.nvmrc)** — run **`nvm install` / `nvm use`** from **`editor/`** (matches **`vscodium/.nvmrc`**; see **[`README.md`](README.md)**). **Canonical helper:** from the repo root, **`source editor/use-node-toolchain.sh`** (or **`./editor/use-node-toolchain.sh node --version`**) — **[`use-node-toolchain.sh`](use-node-toolchain.sh)**.
