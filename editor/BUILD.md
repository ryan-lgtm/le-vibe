...
3. **Stack + IDE (production demo):** from the repository root, **`./packaging/scripts/build-le-vibe-debs.sh --with-ide`** builds **`le-vibe_*_all.deb`** (repo-root **`debian/`**) and then the IDE **`.deb`** when **`VSCode-linux-*`** exists — **[`docs/PM_DEB_BUILD_ITERATION.md`](../docs/PM_DEB_BUILD_ITERATION.md)**; attach both **`*.deb`** and **`SHA256SUMS`** when publishing — **[`docs/apt-repo-releases.md`](../docs/apt-repo-releases.md)** (*IDE package*); default CI **`le-vibe-deb`** is stack-only — **[`docs/PM_STAGE_MAP.md`](../docs/PM_STAGE_MAP.md)** (*H1 vs §7.3 .deb bundles*).
