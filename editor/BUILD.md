...
**Partial tree:** if **`VSCode-linux-*/bin/`** exists but **`codium`** is missing (e.g. only **`codium-tunnel`** after an interrupted build), **`print-built-codium-path.sh`** / **`verify-14c-local-binary.sh`** explain that case — finish **`./dev/build.sh`** per **14.c** (or use a full unpack / **14.f** tarball). **`./packaging/scripts/print-step14-vscode-linux-bin-files.sh`** prints the same comma-separated **`bin/`** list as **`lvibe ide-prereqs --json`** field **`vscode_linux_bin_files`** (also echoed by **`preflight-step14-closeout.sh`** / **`verify-step14-closeout.sh`** when **`vscode_linux_build`** is **`partial`**).
