...
**Partial tree:** if **`VSCode-linux-*/bin/`** exists but **`codium`** is missing (e.g. only **`codium-tunnel`** after an interrupted build), **`print-built-codium-path.sh`** / **`verify-14c-local-binary.sh`** explain that case — finish **`./dev/build.sh`** per **14.c** (or use a full unpack / **14.f** tarball). To **replace** a partial tree with a **`linux_compile`** artifact without recompiling locally, run **`packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh /path/to/vscodium-linux-build.tar.gz`** (add **`--yes`** when stdin is not a TTY) — then **`probe-vscode-linux-build.sh`** should report **`ready`**. **`./packaging/scripts/print-step14-vscode-linux-bin-files.sh`** prints the same comma-separated **`bin/`** list as **`lvibe ide-prereqs --json`** field **`vscode_linux_bin_files`** (also echoed by **`preflight-step14-closeout.sh`** / **`verify-step14-closeout.sh`** when **`vscode_linux_build`** is **`partial`**).
