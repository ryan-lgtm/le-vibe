...
- **Preflight (local):** from the repo root, **`./packaging/scripts/check-linux-vscodium-build-deps.sh`** compares your host to **`build-le-vibe-ide.yml`** *Install Linux build dependencies* and **`pkg-config xkbfile`** — exits **`0`** when aligned, otherwise prints a one-line **`apt-get install`** hint (STEP 14 / §7.3).
