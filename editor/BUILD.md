...
**Partial tree:** if **`VSCode-linux-*/bin/`** exists but **`codium`** is missing (e.g. only **`codium-tunnel`** after an interrupted build), **`print-built-codium-path.sh`** / **`verify-14c-local-binary.sh`** explain that case — finish **`./dev/build.sh`** per **14.c** (or use a full unpack / **14.f** tarball).
