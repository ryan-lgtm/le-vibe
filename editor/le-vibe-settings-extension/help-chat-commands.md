...
| **Help — mentions & commands** | This panel: **`/setup-workspace`**, **`/agent <role_id>`**, **`@<role>`** → **`.lvibe/agents/<id>/skill.md`** using canonical aliases (`@sme`, `@props`, `@prod`, `@be-eng`, `@fe-eng`, `@do-eng`, `@marketing`, `@cs`, `@rev`). |
...
- **`/agent <role_id>`** — Steer using **`.lvibe/agents/<role_id>/skill.md`** (example: **`/agent @prod`**).

## Mentions

- **`@<role>`** — Same idea as **`/agent`**; maps to **`.lvibe/agents/<id>/skill.md`**. Canonical roles: **`@sme`**, **`@props`**, **`@prod`**, **`@be-eng`**, **`@fe-eng`**, **`@do-eng`**, **`@marketing`**, **`@cs`**, **`@rev`**.
