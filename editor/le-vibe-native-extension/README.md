...
**Source on GitHub (task-n26-1):** browse this package in the monorepo tree — **`https://github.com/ryan-lgtm/le-vibe/tree/main/editor/le-vibe-native-extension`** (**`package.json`** **`homepage`**).

**Monorepo clone (task-n28-1):** **`package.json`** **`repository`** — **`type`** **`git`**, **`url`** **`https://github.com/ryan-lgtm/le-vibe.git`**, **`directory`** **`editor/le-vibe-native-extension`** (clone the monorepo, then **`cd`** into that path for npm scripts / packaging from this package).

**License (task-n30-1):** SPDX **`MIT`** — **`package.json`** **`license`** (marketplace / CI identity; same topic as **`OPERATOR.md`** *publisher and license*).

**Publisher (task-n32-1):** **`levibe`** — **`package.json`** **`publisher`** (VS Code extension id uses **`<publisher>.<name>`**; pairs with **License** above).

**Keywords (task-n34-1):** **`package.json`** **`keywords`**: **`levibe`**, **`ollama`**, **`local-first`**, **`vscode-extension`** (registry / marketplace search tags).

**Categories (task-n36-1):** **`package.json`** **`categories`**: **`Other`** (VS Code Marketplace extension category).
...
**Activation sequence return hop (task-n68-1):** from **`Activation docs sequence (task-n60-1)`**, jump back to **`Activation docs quick index (task-n62-1)`** when you need the compact activation-doc entrypoint again.
...
- **CP6 automated acceptance (task-cp6-1):** run **`npm run e2e-acceptance`** (same as verify by default; optional strict Ollama via env — see **`docs/E2E_ACCEPTANCE.md`**). Underlying: **`scripts.e2e-acceptance`** = **`node ./scripts/e2e-acceptance.js`**.
- **Underlying scripts:** **`package.json`** **`scripts.test`** = **`node --test ./test/*.test.js`**; **`scripts.smoke`** = **`node ./scripts/smoke-integration.js`**; **`scripts.e2e-acceptance`** = **`node ./scripts/e2e-acceptance.js`**; **`scripts.package`** = **`vsce package`** (see **`OPERATOR.md`** *Packaged VSIX* for **`npm run package`** output path and **`code --install-extension`**).
