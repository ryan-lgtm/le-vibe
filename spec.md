...
**Full-product maintainer build (STEP 14 / §7.3):** From the monorepo root, **`packaging/scripts/build-le-vibe-debs.sh --with-ide`** builds **`le-vibe_*_all.deb`** and **`le-vibe-ide_*_amd64.deb`** when **`editor/vscodium/VSCode-linux-*`** exists and prints **Full-product install** on success — **[`docs/PM_DEB_BUILD_ITERATION.md`](docs/PM_DEB_BUILD_ITERATION.md)** (*Success output (`--with-ide`)*); close-out gate: **`packaging/scripts/verify-step14-closeout.sh --require-stack-deb`**; install both **`.deb`** files — **[`packaging/debian-le-vibe-ide/README.md`](packaging/debian-le-vibe-ide/README.md)** (*Install both packages*).
