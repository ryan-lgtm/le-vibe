# Lé Vibe — monorepo developer reference

> **Audience:** Maintainers, engineers, and automated doc-contract tests. This file preserves the former root `README.md` narrative (orchestration, CI, packaging depth, E1 anchors). **New visitors:** start at the repository [`README.md`](../README.md).
...
**Master orchestrator STEP 7** (H4 — **[`docs/PM_STAGE_MAP.md`](docs/PM_STAGE_MAP.md)**): reproducible **Continue** installs via Open VSX **`continue.continue@<semver>`**, plus companion **`redhat.vscode-yaml@<semver>`** from **[`packaging/vscode-yaml-openvsx-version`](packaging/vscode-yaml-openvsx-version)**. **`packaging/scripts/install-continue-extension.sh`** consumes both pins (also under **`/usr/share/le-vibe/`** after **`dpkg -i`**). Validate with **`packaging/scripts/verify-continue-pin.sh`** (runs from **`ci-smoke.sh`** before **`pytest`**). Authority **[`docs/continue-extension-pin.md`](docs/continue-extension-pin.md)** (**§14.h** table when **`LE_VIBE_EDITOR`** points at a local **`codium`**). Tests **`test_continue_openvsx_pin.py`**. Package README: **[`le-vibe/README.md`](le-vibe/README.md)** *Continue / Open VSX pin (STEP 7 / H4)*.
...
- **Continue (one command after first-run):** with VSCodium installed, run **`le-vibe-setup-continue`** (on `PATH` from the `.deb`, or `packaging/bin/le-vibe-setup-continue` in a dev tree). It runs **sync** then **extension install** with explicit exit codes:
  ...
  Advanced: run [`packaging/scripts/sync-continue-config.sh`](packaging/scripts/sync-continue-config.sh) and [`packaging/scripts/install-continue-extension.sh`](packaging/scripts/install-continue-extension.sh) separately; default extension ids **`continue.continue`** and **`redhat.vscode-yaml`**. **Roadmap H4:** pin files **[`packaging/continue-openvsx-version`](packaging/continue-openvsx-version)** + **[`packaging/vscode-yaml-openvsx-version`](packaging/vscode-yaml-openvsx-version)** + validator **[`packaging/scripts/verify-continue-pin.sh`](packaging/scripts/verify-continue-pin.sh)** (also enforced by pytest + CI smoke). Override with **`LE_VIBE_CONTINUE_OPENVSX_VERSION=latest`**, **`LE_VIBE_VSCODE_YAML_OPENVSX_VERSION`**, or extension id env vars — see **[`docs/continue-extension-pin.md`](docs/continue-extension-pin.md)**.
