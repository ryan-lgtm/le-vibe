...
| [`editor/README.md`](../editor/README.md), [`editor/BUILD.md`](../editor/BUILD.md) | Build entrypoints; if **`VSCode-linux-*/bin/codium`** is missing, see **BUILD.md** (*Partial tree*), **`./editor/print-built-codium-path.sh`**, **`./editor/print-vsbuild-codium-path.sh`**, **`./packaging/scripts/print-step14-vscode-linux-bin-files.sh`**, [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (*Partial VSCode-linux tree*), **`packaging/scripts/build-le-vibe-ide-deb.sh --help`**, [`apt-repo-releases.md`](apt-repo-releases.md) (*IDE package* — *Incomplete Linux build*). |
