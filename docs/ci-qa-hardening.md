...
**Maintainer CI tarball helpers (14.f — same family as `preflight-step14-closeout.sh` / `lvibe ide-prereqs --print-closeout-commands`):** **`packaging/scripts/print-github-linux-compile-artifact-hint.sh`**, **`packaging/scripts/trigger-le-vibe-ide-linux-compile.sh`**, **`packaging/scripts/download-vscodium-linux-compile-artifact.sh`** (optional **`--install`**), **`packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh`**.
