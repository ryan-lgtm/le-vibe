...
**Preflight (all gaps):** **`packaging/scripts/preflight-step14-closeout.sh`** prints **`[ok]`** / **`[missing]`** for **`ci-editor-gate.sh`**, **14.c** **`editor/vscodium/VSCode-linux-*/bin/codium`**, **`packaging/le-vibe-ide_*.deb`**, and (with **`--require-stack-deb`**) stack **`le-vibe_*.deb`**, then a **`vscode_linux_build:`** line (**`ready`** \| **`partial`** \| **`absent`**) from **`packaging/scripts/probe-vscode-linux-build.sh`** — same **`vscode_linux_build`** field as **`lvibe ide-prereqs --json`** / **`verify-step14-closeout.sh --json`**. On **14.c** failure, **`verify-step14-closeout.sh`** stderr also prints **`vscode_linux_build`** before exiting — run before **`verify-step14-closeout.sh`** when iterating long **`dev/build.sh`** compiles so every blocker is visible at once.
...
| **1** | **`--with-ide`** set but **`packaging/scripts/probe-vscode-linux-build.sh`** is not **`ready`** before the stack **`dpkg-buildpackage`** (unless **`--vs-build`** points at a tree with **`bin/codium`**), the IDE build failed, or **`le-vibe-ide_*.deb`** was not found under **`packaging/`** after the IDE step — see *Failure (`--with-ide`)* below. |
...
**Failure (`--with-ide`):** If **`--with-ide`** is set but **`packaging/scripts/probe-vscode-linux-build.sh`** reports **`partial`**, **`absent`**, or **`unknown`** (and you did not pass **`--vs-build`** to a **`VSCode-linux-*`** tree that already contains **`bin/codium`**), **`build-le-vibe-debs.sh`** exits with status **1** **before** the stack **`dpkg-buildpackage`** — fail fast so you do not spend time on the stack **`.deb`** when **14.c** is incomplete. If **`--with-ide`** is set but **`le-vibe-ide_*.deb`** is not found under **`packaging/`** after the IDE build step, **`build-le-vibe-debs.sh`** also exits with status **1** — a stack-only **`.deb`** is not treated as a successful full-product run.

**Partial VSCode-linux tree:** If **`editor/vscodium/VSCode-linux-*`** exists but **`bin/codium`** is missing (e.g. only **`codium-tunnel`** after an interrupted compile), **`packaging/scripts/build-le-vibe-debs.sh --with-ide`** (without **`--vs-build`**) exits **1** before the stack build after **`probe-vscode-linux-build.sh`** reports **`partial`**; **`packaging/scripts/stage-le-vibe-ide-deb.sh`** and **`./editor/print-built-codium-path.sh`** fail with an explicit hint — finish **`dev/build.sh`** per **`editor/BUILD.md`** (*Partial tree*, **14.c**), or obtain **`linux_compile`** **`vscodium-linux-build.tar.gz`** from **GitHub Actions** (browser **Artifacts** or **`gh`** — **`packaging/scripts/print-github-linux-compile-artifact-hint.sh`**) and unpack into **`editor/vscodium/`** with **`packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh`** (**14.f** — **`editor/BUILD.md`**). **`packaging/scripts/print-step14-vscode-linux-bin-files.sh`** prints the same **`bin/`** inventory as **`lvibe ide-prereqs --json`** **`vscode_linux_bin_files`** (also in **`preflight-step14-closeout.sh`** / **`verify-step14-closeout.sh`** stderr when **14.c** fails and the probe is **`partial`**). **`./editor/print-vsbuild-codium-path.sh`** helps when the tree lives outside **`editor/vscodium/`**; **`packaging/scripts/build-le-vibe-ide-deb.sh --help`** lists the same triage and **`verify-step14-closeout.sh --require-stack-deb`** for full close-out after both **`.deb`**s exist. Same maintainer story — **[`docs/apt-repo-releases.md`](apt-repo-releases.md)** (*IDE package* — *Incomplete Linux build*); **`packaging/debian-le-vibe-ide/README.md`** (*Build steps*).
...
- [ ] `--with-ide` requires **`VSCode-linux-*`** with **`bin/codium`** (or **`--vs-build`** to such a tree); without **`--vs-build`**, **`probe-vscode-linux-build.sh`** must be **`ready`** before stack **`dpkg-buildpackage`** (fail fast when **14.c** is incomplete). If the IDE **`.deb`** is still missing under **`packaging/`**, **`build-le-vibe-debs.sh`** exits **1** (not stack-only success).
