...
| **Maintainer stack + IDE `.deb` (STEP 14 / §7.3)** — not default **`ci.yml`** **`le-vibe-deb`** | [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (**`build-le-vibe-debs.sh --with-ide`**, **Full-product install**, **`verify-step14-closeout.sh --require-stack-deb`** + optional **`--apt-sim`** for explicit dependency simulation); [`apt-repo-releases.md`](apt-repo-releases.md) (*IDE package*); [`PM_STAGE_MAP.md`](PM_STAGE_MAP.md) (*H1 vs §7.3 .deb bundles*) |
