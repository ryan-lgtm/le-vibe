...
**Bootstrap `.deb` (stack):** [`debian/le-vibe.README.Debian`](../debian/le-vibe.README.Debian) → **`/usr/share/doc/le-vibe/README.Debian`** — install the **`le-vibe`** package for Ollama + `lvibe` + **`.lvibe/`**; wire **`LE_VIBE_EDITOR`** to your **`editor/`** build output when ready. **Production pair (STEP 14 / §7.3):** after **`VSCode-linux-*`** exists, from the repo root run **`packaging/scripts/build-le-vibe-debs.sh --with-ide`** ([**`docs/PM_DEB_BUILD_ITERATION.md`**](PM_DEB_BUILD_ITERATION.md)) to build **`le-vibe_*_all.deb`** + **`le-vibe-ide_*_amd64.deb`**; attach both and **`SHA256SUMS`** per **[`docs/apt-repo-releases.md`](apt-repo-releases.md)** (*IDE package*). Default CI **`le-vibe-deb`** is stack-only — **[`docs/PM_STAGE_MAP.md`](PM_STAGE_MAP.md)** (*H1 vs §7.3 .deb bundles*).
