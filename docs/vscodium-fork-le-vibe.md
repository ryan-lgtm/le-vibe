...
**Master orchestrator order:** **0 → 1 → 14 → 2–13 → 15–17** — **`editor/`** / **§7.3** follows **STEP 0–1**, not last. See **[`PROMPT_BUILD_LE_VIBE.md`](PROMPT_BUILD_LE_VIBE.md)** (*ORDERED WORK QUEUE*, *Rolling iteration — prefer continuation*) and **[`PM_STAGE_MAP.md`](PM_STAGE_MAP.md)** *Execution order* / **STEP 16** (same story as **[`editor/README.md`](../editor/README.md)** *Master orchestrator order*).
...
**Bootstrap `.deb` (stack):** [`debian/le-vibe.README.Debian`](../debian/le-vibe.README.Debian) → **`/usr/share/doc/le-vibe/README.Debian`** — install the **`le-vibe`** package for Ollama + `lvibe` + **`.lvibe/`**; wire **`LE_VIBE_EDITOR`** to your **`editor/`** build output when ready. **Production pair (STEP 14 / §7.3):** after **`VSCode-linux-*`** exists, from the repo root **`./packaging/scripts/build-le-vibe-debs.sh --with-ide`** builds **`le-vibe_*_all.deb`** + **`le-vibe-ide_*_amd64.deb`** — **[`docs/PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md)** (*Success output (`--with-ide`)* — **Full-product install** on stdout); **preflight (optional):** **`./packaging/scripts/preflight-step14-closeout.sh --require-stack-deb`** or **`lvibe ide-prereqs --print-closeout-commands`** — **[`docs/PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md)** (*Preflight (all gaps)*); **local close-out gate:** **`./packaging/scripts/verify-step14-closeout.sh --require-stack-deb`** (add **`--apt-sim`** for explicit dependency simulation, **`--json`** for machine-readable close-out output; **`apt_sim_note`**, **`desktop_file_validate`** — **[`docs/PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md)** (*`--json` close-out payload*)); stack **`le-vibe_*.deb`** path — **`packaging/scripts/resolve-latest-le-vibe-stack-deb.sh`** (**[`docs/PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md)** *Output paths*); install both **`.deb`** files — **[`packaging/debian-le-vibe-ide/README.md`](../packaging/debian-le-vibe-ide/README.md)** (*Install both packages*); attach both and **`SHA256SUMS`** per **[`docs/apt-repo-releases.md`](apt-repo-releases.md)** (*IDE package*, *Maintainer build output*). Default CI **`le-vibe-deb`** is stack-only — **[`docs/PM_STAGE_MAP.md`](PM_STAGE_MAP.md)** (*H1 vs §7.3 .deb bundles*).
...
**Partial VSCode-linux tree (missing `bin/codium`):** finish **`cd editor/vscodium && ./dev/build.sh`** per **[`editor/BUILD.md`](../editor/BUILD.md)** (*Partial tree*, **14.c**), or obtain **`linux_compile`** **`vscodium-linux-build.tar.gz`** and vendor it — **`./packaging/scripts/print-github-linux-compile-artifact-hint.sh`**, **`./packaging/scripts/trigger-le-vibe-ide-linux-compile.sh`**, **`./packaging/scripts/download-vscodium-linux-compile-artifact.sh`** **`--install`**, **`./packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh`** (**14.f** — same family as **`lvibe ide-prereqs --print-closeout-commands`** / **`preflight-step14-closeout.sh`**).
...
|------|---------------------|---------------|
| **1. Layout + toolchain gate** | From repo root: **`./editor/smoke.sh`** | **`ci-editor-gate`** reports **`layout=vscodium`** (or your supported layout); **`bash -n`** on upstream scripts + monorepo helpers (**`use-node-toolchain.sh`**, **`fetch-vscode-sources.sh`**, **`print-built-codium-path.sh`**, **`verify-14c-local-binary.sh`**, **`print-vsbuild-codium-path.sh`**, **`print-ci-tarball-codium-path.sh`**, **`print-step14-vscode-linux-bin-files.sh`**, **`smoke-built-codium-lvibe.sh`**, **`ci-vscodium-linux-dev-build.sh`**) OK; **`editor/.nvmrc`** matches **`editor/vscodium/.nvmrc`**. |
