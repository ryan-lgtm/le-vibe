...
**Top priority (product directive):** The **Ubuntu, clone-and-build golden path** comes first. A developer must be able to **`git clone`** this monorepo, **run the documented script(s)**, and **compile the Lé Vibe IDE locally (typically once)**, then **use that build on Ubuntu** with the **full product**: **managed Ollama** and **hardware-aware model selection**, **Continue** extension **installed and pre-configured**, and **orchestration as designed**—the **operator** as the primary chat agent and **specialist/subagent** behavior driven from **`.lvibe/`** (session manifest, skill agents, **`docs/SESSION_ORCHESTRATION_SPEC.md`**). **Broad distribution** (apt publishing, release cadence, multi-OS installers as the main optimization) is **secondary** to making this local path complete and trustworthy; **macOS and Windows** are **later**. Engineering may keep existing orchestrator loops and queues unchanged; this directive **clarifies what “success” optimizes for first**—not a rewrite of process.

**Second priority (north star — once the IDE is locally shippable):** **Specialists and orchestration** are the **core differentiator**—not generic chat. Each **role** ships with a **broad predefined persona**: a short, stable **lens** the model applies to **everything** it learns, **delegates**, and **coordinates** with other roles. For each role, define the **top three to five** priorities that matter most to that discipline; those priorities anchor reasoning and handoffs so flows stay **smooth** and **recognizably “Lé Vibe.”** As the **workspace is learned** **or** the user **edits persona / manifest** in **Lé Vibe configuration inside the editor**, that role **may evolve** for that workspace: specialists are allowed to **reinstruct** themselves from **interaction with other subagents** and the **user**, without losing the guardrails in **`docs/SESSION_ORCHESTRATION_SPEC.md`** and **§5** consent/storage. **Observability:** invest in **live-watching** the **Continue** chat—users should be able to **follow** multi-agent conversation, **see what is being worked on**, and **how** decisions and delegation unfold; product and engineering should treat that experience as **must-ship** alongside the operator/subagent logic itself.
...
**What we are optimizing for:** a **single installable Lé Vibe IDE**—branded **Code - OSS** binary (name, icons, About, desktop entry) built from **`editor/`**, not “upstream VSCodium + our scripts” as the long-term default. For **v1 emphasis**, treat **“installable”** as **installable on the developer’s machine** via the **clone → script → compile** path above; packaging-for-the-world remains valuable but **after** that path is excellent. Roadmap **H6** — **[`docs/vscodium-fork-le-vibe.md`](vscodium-fork-le-vibe.md)** — is **P0**: land upstream sources under **`editor/`**, wire CI, ship artifacts from the **same** git history as the stack.
...
**Release bundles (H1 / STEP 8 vs STEP 14 / §7.3):** Default **`ci.yml`** artifact **`le-vibe-deb`** contains the stack **`le-vibe`** **`.deb`**, SBOM, and **`SHA256SUMS`** for those files — **not** **`le-vibe-ide_*_amd64.deb`**. **`le-vibe-ide_*_amd64.deb`** is built separately (**`packaging/scripts/build-le-vibe-debs.sh --with-ide`**, **`packaging/debian-le-vibe-ide/`**); **`--with-ide`** exits before stack **`dpkg-buildpackage`** unless **`packaging/scripts/probe-vscode-linux-build.sh`** is **`ready`** (or **`--vs-build`** supplies **`bin/codium`**) — **`docs/PM_DEB_BUILD_ITERATION.md`** *Failure (`--with-ide`)*, **`docs/apt-repo-releases.md`** (*IDE package* — *Fail-fast (`build-le-vibe-debs.sh --with-ide`)*); on success that script prints **Full-product install** — **`docs/PM_DEB_BUILD_ITERATION.md`** (*Success output (`--with-ide`)*); **preflight (optional):** **`packaging/scripts/preflight-step14-closeout.sh --require-stack-deb`** or **`lvibe ide-prereqs --print-closeout-commands`** ( **`packaging/scripts/probe-vscode-linux-build.sh`**, preflight, verify lines) — **`docs/PM_DEB_BUILD_ITERATION.md`** (*Preflight (all gaps)*); **local close-out gate:** **`packaging/scripts/verify-step14-closeout.sh --require-stack-deb`** (add **`--apt-sim`** for explicit dependency simulation, **`--json`** for machine-readable close-out output; **`apt_sim_note`** — **`docs/PM_DEB_BUILD_ITERATION.md`** (*`--json` close-out payload*)); **partial VSCode-linux tree (missing `bin/codium`):** **`docs/PROMPT_BUILD_LE_VIBE.md`** (*Partial VSCode-linux tree*); **`debian/lvibe.1`** (**COMMANDS** for **`lvibe ide-prereqs --print-closeout-commands`**); **`editor/print-ci-tarball-codium-path.sh`** (**14.f** temp unpack). install both **`.deb`** files — **`packaging/debian-le-vibe-ide/README.md`** (*Install both packages*). Full-product publishes attach both — **`docs/apt-repo-releases.md`** (*IDE package*, *Maintainer build output*); **`docs/PM_STAGE_MAP.md`** (*H1 vs §7.3 .deb bundles*). **Ordering:** **build machine** close-out, **test host** install/smoke — **`docs/apt-repo-releases.md`** (*IDE package*).

### Product and project management — in service of the IDE

Session manifests, **`.lvibe/`** epics/tasks, skill agents, and orchestration docs exist to **coordinate shipping the Lé Vibe IDE and stack as one product**—not as unrelated backlogs. The **Master orchestrator** and lazy prompts in **`docs/PROMPT_BUILD_LE_VIBE.md`** stay as-is; teams should still read this **Prioritization** section so work **aligns with the clone-and-compile Ubuntu golden path**, then with **persona-hardened specialists**, **evolving workspace roles**, and **live-watching** in **Continue** as laid out above. Material choices for **persona schema**, **editor-side configuration surfaces**, and **chat observability** use **§7.2** when specs do not uniquely decide.
