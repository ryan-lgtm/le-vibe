...
| PM iteration tracks | [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (**`build-le-vibe-debs.sh --with-ide`**, **Full-product install**, **`verify-step14-closeout.sh --require-stack-deb`**; **`le-vibe-deb`** = stack-only in default CI), [`PM_IDE_SETTINGS_AND_WORKFLOWS.md`](PM_IDE_SETTINGS_AND_WORKFLOWS.md) |
