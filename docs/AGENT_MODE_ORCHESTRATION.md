...
| PM iteration tracks | [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (**`build-le-vibe-debs.sh --with-ide`**, **Full-product install**, **`preflight-step14-closeout.sh`** / **`lvibe ide-prereqs --print-closeout-commands`** (*Preflight (all gaps)*), **`verify-step14-closeout.sh --require-stack-deb`** + optional **`--apt-sim`** / **`--json`** (**`apt_sim_note`** — *`--json` close-out payload*); **`le-vibe-deb`** = stack-only in default CI; **Partial VSCode-linux** — [`editor/BUILD.md`](../editor/BUILD.md) (*Partial tree*), *Partial VSCode-linux tree* in same doc, **`./editor/print-built-codium-path.sh`**, **`./editor/print-vsbuild-codium-path.sh`**, **`./packaging/scripts/print-step14-vscode-linux-bin-files.sh`**, **`./packaging/scripts/print-github-linux-compile-artifact-hint.sh`**, **`./packaging/scripts/trigger-le-vibe-ide-linux-compile.sh`**, **`./packaging/scripts/download-vscodium-linux-compile-artifact.sh`**, **`./packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh`**, **`./editor/print-ci-tarball-codium-path.sh`**, **`debian/lvibe.1`** (**`lvibe ide-prereqs --print-closeout-commands`** / **COMMANDS**), **`./packaging/scripts/build-le-vibe-ide-deb.sh --help`**), [`PM_IDE_SETTINGS_AND_WORKFLOWS.md`](PM_IDE_SETTINGS_AND_WORKFLOWS.md) |
