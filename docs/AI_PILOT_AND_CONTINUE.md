...
**Maintainer full-product `.deb` (STEP 14 / §7.3):** **[`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md)** — **`packaging/scripts/build-le-vibe-debs.sh --with-ide`** prints **Full-product install** on success; local close-out gate: **`packaging/scripts/verify-step14-closeout.sh --require-stack-deb`**; default **[`ci.yml`](../.github/workflows/ci.yml)** artifact **`le-vibe-deb`** is **stack-only** — **[`apt-repo-releases.md`](apt-repo-releases.md)** (*IDE package*); **[`PM_STAGE_MAP.md`](PM_STAGE_MAP.md)** (*H1 vs §7.3 .deb bundles*).
