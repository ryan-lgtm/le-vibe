...
**Full-product maintainer `.deb`:** [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) — **`packaging/scripts/build-le-vibe-debs.sh --with-ide`** exits before stack **`dpkg-buildpackage`** unless **`packaging/scripts/probe-vscode-linux-build.sh`** is **`ready`** (or **`--vs-build`** supplies **`bin/codium`**) — *Failure (`--with-ide`)* in the same doc; on success prints **Full-product install** (stack + IDE); **preflight (optional):** **`packaging/scripts/preflight-step14-closeout.sh --require-stack-deb`** or **`lvibe ide-prereqs --print-closeout-commands`** — [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (*Preflight (all gaps)*); **close-out gate:** **`packaging/scripts/verify-step14-closeout.sh --require-stack-deb`** (add **`--apt-sim`** for explicit dependency simulation, **`--json`** for machine-readable close-out output; **`apt_sim_note`**, **`desktop_file_validate`** — [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (*`--json` close-out payload*)); stack **`le-vibe_*.deb`** path — **`packaging/scripts/resolve-latest-le-vibe-stack-deb.sh`** (*Output paths* in the same doc); default **`ci.yml`** artifact **`le-vibe-deb`** is stack-only — [`apt-repo-releases.md`](apt-repo-releases.md) (*IDE package*). **Ordering:** **build machine** close-out, **test host** install/smoke — [`apt-repo-releases.md`](apt-repo-releases.md) (*IDE package*); on the test host after **`apt install`**, **`packaging/scripts/manual-step14-install-smoke.sh`** (**`--json`** — **`desktop_file_validate_on_path`**; **`--verify-only`** — optional **`desktop-file-validate`** on **`le-vibe.desktop`** — same Freedesktop story as **preflight** / **verify** / **`build-le-vibe-ide-deb`**) — [`apt-repo-releases.md`](apt-repo-releases.md) (*IDE package*), [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (*Success output (`--with-ide`)*).

**Partial VSCode-linux tree (missing `bin/codium`):** [`editor/BUILD.md`](../editor/BUILD.md) (*Partial tree*, **14.c**); **`./editor/print-built-codium-path.sh`**, **`./editor/print-vsbuild-codium-path.sh`**; **`./packaging/scripts/print-step14-vscode-linux-bin-files.sh`** (same **`bin/`** list as **`lvibe ide-prereqs --json`** **`vscode_linux_bin_files`**; also in **`preflight-step14-closeout.sh`** / **`verify-step14-closeout.sh`** when **`probe-vscode-linux-build.sh`** is **`partial`**); **`./packaging/scripts/print-github-linux-compile-artifact-hint.sh`** (CI **`linux_compile`** tarball — browser **Actions** or **`gh`**); **`./packaging/scripts/trigger-le-vibe-ide-linux-compile.sh`** (**workflow_dispatch** **linux_compile** — **`GITHUB_TOKEN`** with **repo** scope); **`./packaging/scripts/download-vscodium-linux-compile-artifact.sh`** (same tarball via GitHub REST API + **`GITHUB_TOKEN`** / **`GH_TOKEN`** — optional **`--install`**); **`./packaging/scripts/install-vscodium-linux-tarball-to-editor-vendor.sh`** (**14.f** — vendor into **`editor/vscodium/`**); **`./editor/print-ci-tarball-codium-path.sh`** (**14.f** — temp unpack); **`lvibe ide-prereqs --print-closeout-commands`** (partial/absent/ready blocks — **`debian/lvibe.1`** **COMMANDS**); [`PM_DEB_BUILD_ITERATION.md`](PM_DEB_BUILD_ITERATION.md) (*Partial VSCode-linux tree*); **`./packaging/scripts/build-le-vibe-ide-deb.sh --help`**; [`apt-repo-releases.md`](apt-repo-releases.md) (*IDE package* — *Incomplete Linux build*).
