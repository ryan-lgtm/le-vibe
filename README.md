...
**Lé Vibe IDE (Roadmap H6, STEP 14):** the **stack** job above does **not** compile the **Electron** editor. **[`build-le-vibe-ide.yml`](.github/workflows/build-le-vibe-ide.yml)** (and optional manual **[`build-linux.yml`](.github/workflows/build-linux.yml)**) check out **`editor/vscodium`**, run layout/metadata smoke and script syntax checks, and upload small **metadata** artifacts—see **`editor/README.md`** and **`docs/ci-qa-hardening.md`**.
