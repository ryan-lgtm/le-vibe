...
**Lé Vibe IDE (Roadmap H6, STEP 14):** default **CI** does **not** compile the **Electron** editor, but it **does** run the vendoring smoke above via **`ci-smoke.sh`**. **[`build-le-vibe-ide.yml`](.github/workflows/build-le-vibe-ide.yml)** (and optional manual **[`build-linux.yml`](.github/workflows/build-linux.yml)**) re-run the same gate with **`editor/**`-scoped **PR** triggers and upload small **metadata** artifacts. **Local parity:** from the repo root, **`./editor/smoke.sh`** runs the same gate—see **`editor/README.md`**, **`docs/ci-qa-hardening.md`** (*IDE smoke*), and **`docs/PRODUCT_SPEC.md`** (*Prioritization* — *How to sequence work*).
