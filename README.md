...
**Lé Vibe IDE (Roadmap H6, STEP 14):** the **stack** job above does **not** compile the **Electron** editor. **[`build-le-vibe-ide.yml`](.github/workflows/build-le-vibe-ide.yml)** (and optional manual **[`build-linux.yml`](.github/workflows/build-linux.yml)**) check out **`editor/vscodium`**, run layout/metadata smoke and script syntax checks, and upload small **metadata** artifacts. **Local parity:** from the repo root, **`./editor/smoke.sh`** runs the same gate when the submodule is present—see **`editor/README.md`**, **`docs/ci-qa-hardening.md`** (*IDE smoke*), and **`docs/PRODUCT_SPEC.md`** (*Prioritization* — *How to sequence work*).
