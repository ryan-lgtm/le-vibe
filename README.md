...
**Honesty (14.d):** that fast **`ci-editor-gate`** / **`./editor/smoke.sh`** path checks vendoring + script parity only — not Lé Vibe–visible product branding in the Electron shell. Staging map **[`editor/le-vibe-overrides/branding-staging.checklist.md`](editor/le-vibe-overrides/branding-staging.checklist.md)** — read *PRODUCT_SPEC §7.2 (read before overrides)* first ([**`docs/PRODUCT_SPEC.md`**](docs/PRODUCT_SPEC.md) §7.2); see **`editor/README.md`** *14.c vs 14.d*.
