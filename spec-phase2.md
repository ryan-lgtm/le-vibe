...
| **Single “Lé Vibe IDE” shell (Code OSS + branding)** | **`editor/vscodium`** — **VSCodium** git submodule; **`editor/le-vibe-overrides/`** — placeholder for Lé Vibe–only layers; IDE **CI** (layout gate, **`bash -n`**, **`.nvmrc`** sync, PR paths). **`LE_VIBE_EDITOR`** still points at **VSCodium**/system until a **Lé Vibe–branded** binary exists | **Branding** wired into the build (icons, About, product name — **§7.2** where material), automated **get_repo**/**build**, **Linux** artifacts — **H6** — `docs/vscodium-fork-le-vibe.md`, **`editor/README.md`**, **`editor/BUILD.md`**, **`editor/VENDORING.md`** |
